...
(() => {
  ...
  async function normalize(file) {
    ...
    try {
      ...
      if (!blob) throw new Error("Could not prepare this picture.");
      return blob;
  ...
  function open({ user, display = user, token = "" }) {
    ...
    fileInput.addEventListener("change", async () => {
      ...
      try {
        ...
        const image = document.createElement("img"); image.src = previewUrl; image.alt = "New profile picture preview";
        preview.replaceChildren(image); message("Ready to save."); controls();
